                            ...
                            <div class="p-5">
                                ...
                                <div class="mt-2">
                                    <div class="flex flex-wrap timeSlotRow">
                                        ...
                                        @if ($loop->first)
                                        <input type="hidden" name="time" value="{{ $slot_time==''? $today_timeslot['start_time'] : $slot_time }}">
                                        @endif
                                        <a href="javascript:void(0)" onclick="thisTime({{ $loop->iteration }})" class="time timing{{ $loop->iteration }} border border-gray text-center py-1 2xl:text-sm 2xl:px-2 sm:text-sm sm:px-2 msm:text-sm msm:px-2 leading-4 font-fira-sans font-normal xl:text-xs xl:px-1 xlg:text-xs xlg:px-1 xlg:w-28 lg:w-28 xsm:w-28 xxsm:w-28 text-black m-1 {{ ($today_timeslot['start_time']==$slot_time || ($slot_time=='' &&  $loop->first)) ? 'activeTimeslots' : '' }}">
