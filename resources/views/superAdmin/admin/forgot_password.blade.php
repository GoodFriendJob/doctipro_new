        ...
        <div class="col-lg-4 col-md-6 col-12 order-lg-1 min-vh-100 order-2 bg-white">
            <div class="p-4 m-3">
                ...
                <div class="text-center mt-5">
                    <a href="{{ url('/login') }}">{{__('Remember Password ?')}}</a>
                </div>
                @endif
                @if ($from =='doctor')
                <div class="text-center mt-5">
                    <a href="{{ url('doctor/doctor_login') }}">{{__('Remember Password ?')}}</a>
                </div>
                @endif
                @if ($from =='pharmacy')
                <div class="text-center mt-5">
                    <a href="{{ url('pharmacy_login') }}">{{__('Remember Password ?')}}</a>
                </div>
                @endif
                @if ($from =='lab')
                <div class="text-center mt-5">
                    <a href="{{ url('pathologist_login') }}">{{__('Remember Password ?')}}</a>
                </div>
                @endif
            </div>
        </div>
        <div class="col-lg-8 col-12 order-lg-2 order-1 min-vh-100 background-walk-y position-relative overlay-gradient-bottom" data-background="{{ url('assets/img/Mobile.png') }}">
