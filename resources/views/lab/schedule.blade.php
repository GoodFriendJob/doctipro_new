...
<section class="section">
    @include('layout.breadcrumb',[
        'title' => __('Lab schedule'),
    ])

        <input type="hidden" name="start_time" value="{{ $lab->start_time }}">
        <input type="hidden" name="end_time" value="{{ $lab->end_time }}">
                    ...
                    <div class="card schedule-widget mb-0">
                        <div class="schedule-header">
                            <div class="schedule-nav">
                                <ul class="nav nav-tabs nav-justified">
                                    @foreach ($lab->workingHours as $working)
                                        <li class="nav-item">
                                            <a class="nav-link {{ $loop->iteration == 1 ? 'active' : '' }}" onclick="display_lab_timeslot({{ $working->id }})" data-toggle="tab" href="#slot_sunday">{{ $working->day_index }}</a>
                                        </li>
                                    @endforeach
                                </ul>
                            </div>
                        ...
                        <div class="tab-content">
                            <div id="slot_sunday" class="tab-pane fade show active">
                                <input type="hidden" name="working_id" value="{{$lab->firstHours->id}}">
                                <h6 class="card-title d-flex float-right justify-content-between">
                                    <a class="edit-link btn btn-danger float-right mt-5" data-toggle="modal" onclick="edit_lab_timeslot()" href="#edit_time_slot">
                                        <i class="fa fa-edit mr-1"></i> {{__('Edit Slot')}}
                                    </a>
                                </h4>
                                @foreach (json_decode($lab->firstHours->period_list) as $list)
            ...
            <form action="" method="post" class="working_form" class="myform">
                ...
                <div class="modal-body">
                    ...
                    <div class="add-more mb-3">
                        <a href="javascript:void(0);" class="add-hours"><i class="fa fa-plus-circle"></i> {{__('Add More')}}</a>
                    </div>
                </div>
